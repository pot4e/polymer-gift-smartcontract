...
contract XGif is CustomChanIbcApp {
    ...
    mapping(address => string[]) public giftLinksOf;
    ...
    function deposit(
        ...
        sendPacket(
            channelId,
            timeoutSeconds,
            abi.encode(msg.sender, packetId, IbcPacketType.DEPOSIT, data)
        );
    }
    ...
    function createGift(
        ...
        bytes memory data = abi.encode(newGift);

        // Send packet
        sendPacket(
            channelId,
            timeoutSeconds,
            abi.encode(msg.sender, giftId, IbcPacketType.DEPOSIT, data)
        );
        return giftId;
    }
    ...
    function onRecvPacket(
        ...
        (
            ...
            bytes memory data
        ) = abi.decode(packet.data, (address, bytes32, IbcPacketType, bytes));

        if (packetType == IbcPacketType.DEPOSIT) {
            onRecvDepositPacket(
                _caller,
                _id,
                abi.decode(data, (IbcPacketBalance))
            );
    ...
    function onRecvDepositPacket(
        address caller,
        bytes32 id,
        IbcPacketBalance memory balance
    ) internal {
        depositPackets[id].ibcStatus = IbcPacketStatus.SENT;
        balancesOf[caller] += balance.amount;
    }

    function onRecvGiftPacket(
        address caller,
        bytes32 id,
        Gift memory gif
    ) internal {
        string memory giftId = bytes32ToString(id);
        gifts[giftId] = gif;
